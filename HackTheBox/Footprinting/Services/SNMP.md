...
```shell-session
4a6f62@htb[/htb]$ cat /etc/snmp/snmpd.conf | grep -v "#" | sed -r '/^\s*$/d'
...
```shell-session
4a6f62@htb[/htb]$ snmpwalk -v2c -c public 10.129.14.128
...
```shell-session
4a6f62@htb[/htb]$ sudo apt install onesixtyone
4a6f62@htb[/htb]$ onesixtyone -c /opt/useful/seclists/Discovery/SNMP/snmp.txt 10.129.14.128
...
```shell-session
4a6f62@htb[/htb]$ sudo apt install braa
4a6f62@htb[/htb]$ braa <community string>@<IP>:.1.3.6.*   # Syntax
4a6f62@htb[/htb]$ braa public@10.129.14.128:.1.3.6.*
