...
```shell-session
4a6f62@htb[/htb]$ cat /etc/ssh/sshd_config  | grep -v "#" | sed -r '/^\s*$/d'
...
```shell-session
4a6f62@htb[/htb]$ git clone https://github.com/jtesta/ssh-audit.git && cd ssh-audit
4a6f62@htb[/htb]$ ./ssh-audit.py 10.129.14.132
...
```shell-session
4a6f62@htb[/htb]$ ssh -v cry0l1t3@10.129.14.132
...
```shell-session
4a6f62@htb[/htb]$ ssh -v cry0l1t3@10.129.14.132 -o PreferredAuthentications=password
...
```shell-session
4a6f62@htb[/htb]$ sudo nmap -sV -p 873 127.0.0.1
...
```shell-session
4a6f62@htb[/htb]$ nc -nv 127.0.0.1 873
...
```shell-session
4a6f62@htb[/htb]$ rsync -av --list-only rsync://127.0.0.1/dev
...
```shell-session
4a6f62@htb[/htb]$ cat /etc/hosts.equiv
...
```shell-session
4a6f62@htb[/htb]$ sudo nmap -sV -p 512,513,514 10.0.17.2
...
```shell-session
4a6f62@htb[/htb]$ rlogin 10.0.17.2 -l htb-student
...
```shell-session
4a6f62@htb[/htb]$ rusers -al 10.0.17.5
